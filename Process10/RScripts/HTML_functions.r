...
MakeIGVSessionXML <- function(files,names,igvdirectory,XMLname,genomeName,locusName="All"){
  ...
  GlobalNode <- newXMLNode("Global",attrs=c(genome=genomeName,locus=locusName,version=3))
  ResourcesNode <- newXMLNode("Resources",parent=GlobalNode)
  for(i in 1:length(resources)){
    resources[[i]] <-  newXMLNode("Resource",parent=ResourcesNode,attrs=c(label=names[i],name=names[i],path=relativePath(files[i],Output),relativePath=T))
    saveXML(GlobalNode,file=Output)
  }
  return(Output)
}
makeIGVLink <- function(files,names,igvdirectory,htmldirectory,XMLname,genomeName,locusName="All"){
  fileSessionName <- MakeIGVSessionXML(files,names,igvdirectory,XMLname,genomeName,locusName="All")
...
makeIGVLinkFresh <- function(files,names,igvdirectory,htmldirectory,XMLname,genomeName,locusName="All"){
  fileSessionName <- MakeIGVSessionXML(files,names,igvdirectory,XMLname,genomeName,locusName="All")
  fileSessionName <- relativePath(fileSessionName,htmldirectory)
...
MakeSSRS <- function(SampleSheet,genome,p){
...
BamIGVLinks[i] <- makeIGVLink(Bams[i],SampleSheet[i,"SampleName"],file.path(WkgDir,"HTML_Report","IGV"),file.path(WkgDir,"HTML_Report","HTMLReport.html"),paste("Bams",gsub("_Processed.bam$","",basename(Bams[i])),sep=""),genomeName=genome,locusName="All") 
BamIGVLinksFresh[i] <- makeIGVLinkFresh(Bams[i],SampleSheet[i,"SampleName"],file.path(WkgDir,"HTML_Report","IGV"),file.path(WkgDir,"HTML_Report","HTMLReport.html"),paste("Bams",gsub("_Processed.bam$","",basename(Bams[i])),sep=""),genomeName=genome,locusName="All")

}



  SSRS1 <- SampleSheet[,c("GenomicsID","SampleName")]
  SSRS2 <- cbind(BamIGVLinks,BamIGVLinksFresh) 
  
  SSRS3 <- SampleSheet[,c("Run","Lane","Tissue","Factor","Antibody","Condition_1","Condition_2","Replicate","Original","delRand","Excluded","Filtered","Unique","DuplicationRate")]
  SSRS <- cbind(SSRS1,SSRS2,SSRS3)
  colnames(SSRS) <- c("GenomicsID","SampleName","IGV_Session_Link","Fresh_IGV_Session_Link","Run","Lane","Tissue","Factor","Antibody","Condition_1","Condition_2","Replicate","Original","delRand","Excluded","Filtered","Unique","DuplicationRate")
	SSRS <- SSRS[,c("SampleName","Tissue","Factor","Condition_1","Original","delRand","Excluded","Filtered","Unique","DuplicationRate","IGV_Session_Link","Fresh_IGV_Session_Link")]
  Gvis_RS <- gvisTable(SSRS, options=list(width=1750, height=35*nrow(SampleSheet)))
  Gvis_RS <- ReformatVisJS(Gvis_RS)  
  
  
  cat(createGoogleGadget(Gvis_RS),file=p)
...
BWIGVLinks[i] <- makeIGVLink(BigWigs[i],SampleSheet[i,"SampleName"],file.path(WkgDir,"HTML_Report","IGV"),file.path(WkgDir,"HTML_Report","HTMLReport.html"),paste("BigWig_",gsub(".bw$","",basename(BigWigs[i])),sep=""),genomeName=genome,locusName="All") 
BWIGVLinksFresh[i] <- makeIGVLinkFresh(BigWigs[i],SampleSheet[i,"SampleName"],file.path(WkgDir,"HTML_Report","IGV"),file.path(WkgDir,"HTML_Report","HTMLReport.html"),paste("BigWig_",gsub(".bw$","",basename(BigWigs[i])),sep=""),genomeName=genome,locusName="All") 
...
"Condition_2","SSD_Of_Coverage","Gini_Of_Coverage","adjusted_Gini_Of_Coverage","BedGraph_Files","BigWig_Files")]
...
"Condition_2","SSD_Of_Coverage","Gini_Of_Coverage","adjusted_Gini_Of_Coverage","BigWig_To_New_IGV","BigWig_To_IGV_Session")
SSCT <- SSCT[,c("SampleName","Tissue","Factor","Condition_1",
"SSD_Of_Coverage","Gini_Of_Coverage","adjusted_Gini_Of_Coverage","BigWig_To_New_IGV","BigWig_To_IGV_Session")]
...
MakeSSMP <- function(SampleSheet,genome,p){
...
MacsIGVLinks[i] <- makeIGVLink(Macs[i],SampleSheet[i,"SampleName"],file.path(WkgDir,"HTML_Report","IGV"),file.path(WkgDir,"HTML_Report","HTMLReport.html"),paste("Macs",gsub("_Processed_peaks.bed$","",basename(Macs[i])),sep=""),genomeName=genome,locusName="All")
MacsIGVLinksFresh[i] <- makeIGVLinkFresh(Macs[i],SampleSheet[i,"SampleName"],file.path(WkgDir,"HTML_Report","IGV"),file.path(WkgDir,"HTML_Report","HTMLReport.html"),paste("Macs",gsub("_Processed_peaks.bed$","",basename(Macs[i])),sep=""),genomeName=genome,locusName="All")
