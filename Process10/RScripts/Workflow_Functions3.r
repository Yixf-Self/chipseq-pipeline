...
if(CallProfilesCheck(Caller,WkgDir,Config)){
  ...
  if(nrow(JustOfInterest) > 0){
...
#if(Caller %in% "Macs"){
      SampleSheet <- ReadAndLock(file.path(WkgDir,"SampleSheet.csv"),WkdDir,SAF=F,napTime=5)
      PeakGenes <- dir(path=file.path(PeakDirectory),pattern="*Annotated.xls$")
      PeakGenesFullPath <- dir(path=file.path(PeakDirectory),pattern="*Annotated.xls$",full.names=T)
    for(i in 1:nrow(SampleSheet)){
    	if(length(grep(gsub(".bwa.homo_sapiens.bam","",SampleSheet[i,"Source_File"]),PeakGenes)) > 0){
    		MacsToAnnotate <- PeakGenesFullPath[grep(gsub(".bwa.homo_sapiens.bam","",SampleSheet[i,"Source_File"]),PeakGenes)]
    		SampleSheet[i,paste(Caller,"_Genes_Annotation",sep="")] <- MacsToAnnotate
    		print("Hello")
    	}
    	if(length(grep(SampleSheet[i,"GenomicsID"],PeakGenes)) > 0){
        MacsToAnnotate <- PeakGenesFullPath[grep(SampleSheet[i,"GenomicsID"],PeakGenes)]
        SampleSheet[i,paste(Caller,"_Genes_Annotation",sep="")] <- MacsToAnnotate
    		print("Hello")
    	}
    }
    
        PeakGenes <- dir(path=file.path(PeakDirectory),pattern="*Annotated.summary$")
    ...
    PeaksGO <- dir(path=file.path(PeakDirectory),pattern="*_GO_Results.txt$")
    PeaksGOFullPath <- dir(path=file.path(PeakDirectory),pattern="*_GO_Results.txt$",full.names=T)    
    for(i in 1:nrow(SampleSheet)){
      if(length(grep(gsub(".bwa.homo_sapiens.bam","",SampleSheet[i,"Source_File"]),PeaksGO)) > 0){
    		PeaksGOToAnnotateSum <- PeaksGOFullPath[grep(gsub(".bwa.homo_sapiens.bam","",SampleSheet[i,"Source_File"]),PeaksGO)]
    	...
    	if(length(grep(SampleSheet[i,"GenomicsID"],PeaksGO)) > 0){
    		PeaksGOToAnnotateSum <- PeaksGOFullPath[grep(SampleSheet[i,"GenomicsID"],PeaksGO)]
    		DataInAnno <- read.delim(PeaksGOToAnnotateSum,sep="\t",comment.char="#",header=T)
    		SampleSheet[i,paste(Caller,"_GO_Terms",sep="")] <- nrow(DataInAnno[DataInAnno[,"Adjusted_P_Value"] < 0.01 & !is.na(DataInAnno[,"Adjusted_P_Value"]),])
    		SampleSheet[i,paste(Caller,"_GO_Table",sep="")] <- PeaksGOToAnnotateSum
  ...
  names(Specialisations) <- namesForSpecials
      ...
      for(i in 1:nrow(SampleSheet)){
        for(k in 1:length(PeakCallerCombos)){
          ...
          Temp2 <- Temp[grep(SampleSheet[i,1],Temp)]
          if(length(Temp2) > 0){
            if(!any(is.na(Temp2))){
             
             
              Results <- read.delim(file.path(WkgDir,"Peaks","Across_Peaks",Temp2),sep="\t",h=F)
              SampleSheet[i,paste("Jaccard_Score_Of_BasePair_Overlap",PeakCallerCombos[k],sep="_")] <- Results[8,2]
              SampleSheet[i,paste("Jaccard_Score_Of_Peaks_Overlap",PeakCallerCombos[k],sep="_")] <- Results[7,2]            
              SampleSheet[i,paste("Percent_Of_Intersection_Peaks_In",strsplit(PeakCallerCombos[k],"_Vs_")[[1]][1],"for",PeakCallerCombos[k],sep="_")] <- Results[5,2]
              SampleSheet[i,paste("Percent_Of_Intersection_Peaks_In",strsplit(PeakCallerCombos[k],"_Vs_")[[1]][2],"for",PeakCallerCombos[k],sep="_")] <- Results[6,2] 
              SampleSheet[i,paste("CoOccurence_BedFile_For",PeakCallerCombos[k],sep="_")] <- gsub("_Res.txt",".bed",file.path(WkgDir,"Peaks","Across_Peaks",Temp2))              
            }
          }
          #SampleSheet[i,PeakCallerCombos[k]] 
        }      
      }
      WriteAndUnlock(SampleSheet,file.path(WkgDir,"SampleSheet.csv"))	
  SampleSheet
