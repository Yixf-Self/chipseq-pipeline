...
Coverage <- SampleAnno[,"Coverage_Fragment_Length"]-ReadLengthActual
CrossCor <- SampleAnno[,"CC_Fragment_Length"]
...
namesForMat[i]  <- ss[gsub(".bwa.homo_sapiens.bam","",ss[,"Source_File"]) %in% FileNames[i],"SampleName"]
ReadAmountsForMat[i]  <- ss[gsub(".bwa.homo_sapiens.bam","",ss[,"Source_File"]) %in% FileNames[i],"Filtered"]
...
for(i in 1:nrow(SampleSheet)){
   if(length(grep(gsub(".bwa.homo_sapiens.bam","",ss[i,"Source_File"]),PeakLocations)) > 0){
     Peaks[i] <-  PeakLocations[grep(gsub(".bwa.homo_sapiens.bam","",ss[i,"Source_File"]),PeakLocations)]
   }else{
     Peaks[i] <- ""
   }
 }
...
if(length(grep("index.html",dir(path=file.path(WkgDir,"Peaks",Caller,"Motif",SampleSheet[i,1],"Denovo")))) > 0){
ForSummaryDenovoMotifFile[i] <- file.path("..","Peaks",Caller,"Motif",SampleSheet[i,1],"Denovo","index.html")
}
if(length(grep("ame.html",dir(path=file.path(WkgDir,"Peaks",Caller,"Motif",SampleSheet[i,1],"Known")))) > 0){
ForSummaryKnownMotifFile[i] <- file.path("..","Peaks",Caller,"Motif",SampleSheet[i,1],"Known","ame.html")
}
if(length(grep("dreme.html",dir(path=file.path(WkgDir,"Peaks",Caller,"Motif",SampleSheet[i,1],"Denovo","dreme_out")))) > 0){
ForSummaryDremeMotifFile[i] <- file.path("..","Peaks",Caller,"Motif",SampleSheet[i,1],"Denovo","index.html")
}
if(length(grep("meme.html",dir(path=file.path(WkgDir,"Peaks",Caller,"Motif",SampleSheet[i,1],"Denovo","meme_out")))) > 0){
ForSummaryMemeMotifFile[i] <- file.path("..","Peaks",Caller,"Motif",SampleSheet[i,1],"Denovo","index.html")
}
}
...
SSMP4 <- SSMP3[,c("GenomicsID","SampleName","Run","Lane","Tissue","Factor","Antibody","Condition_1","Condition_2",paste(Caller,c("Peaks","Genes_In_Peaks","Peaks_In_Genes","GO_Terms","Peaks_Files","Genes_Annotation","GO_Table","Known_Significant_Motifs","Meme_Significant_Motifs"),sep="_"),"IGV_Link_Peaks","Fresh_IGV_Link_Peaks","Total_Peaks_In10000upstream_To_5000upstream","Total_Peaks_In5000upstream_To_2000upstream","Total_Peaks_In2000upstream_To_500upstream","Total_Peaks_In500upstream_To_500downstream","Total_Peaks_In500downastream_To_2000downstream","Total_Peaks_In2000downstream_To_Gene_End","Total_Peaks_InIntragenic","Percentage_Peaks_In10000upstream_To_5000upstream","Percentage_Peaks_In5000upstream_To_2000upstream","Percentage_Peaks_In2000upstream_To_500upstream","Percentage_Peaks_In500upstream_To_500downstream","Percentage_Peaks_In500downastream_To_2000downstream","Percentage_Peaks_In2000downstream_To_Gene_End","Percentage_Peaks_InIntragenic","Total","Total_In_Peaks","Total_Outside_Peaks",
...
MakePeaksHTMLPart <- function(SampleSheet,p,Caller){
hwrite(hwrite(paste("Table  - Number of Peaks Calleds and Genes With Peaks using ",Caller," peak calling algorithm",sep=""),name="T4"),heading=3,p,center=TRUE)
...
Divs1 <- GetDiffDivsAndSpan("Reads and Coverage in Peaks",'Read_In_Peaks')
...
Divs1 <- GetDiffDivsAndSpan("GC content in Peaks",'Read_In_Peaks')
cat(file=p,paste(Divs1[[1]][1],Divs1[[2]][1],Divs1[[3]][1],sep=""))
...
Divs1 <- GetDiffDivsAndSpan("Positive/Negative read ratio in Peaks",'Read_In_Peaks')
cat(file=p,paste(Divs1[[1]][1],Divs1[[2]][1],Divs1[[3]][1],sep=""))
...
peak sets and gene function.In figures 16-18 the frequency, percentage and distibution of peaks in gene ands intragenic regions shown.",br=T,p)
hwrite("",br=T,p)

Divs1 <- GetDiffDivsAndSpan("Peaks in Genes",'Peaks_In_Peaks')
cat(file=p,paste(Divs1[[1]][1],Divs1[[2]][1],Divs1[[3]][1],sep=""))
...
addImage("AveragePeakTSSPlot.png",file.path(WkgDir,"HTML_Report","Plots",paste(Caller,"AveragePeakTSSPlot.png",sep="_")),"","",p)
