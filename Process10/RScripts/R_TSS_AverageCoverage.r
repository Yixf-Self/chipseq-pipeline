...
#     TempCoverage <- coverage(ranges(TempReadRanges),width=seqlengths(TempReadRanges)[names(seqlengths(TempReadRanges)) %in% unique(seqnames(TSS_3))[i]])
     ...
     Temp <- mclapply(unique(seqnames(TSS_3)),deltaConvolve,TSS_3=TSS_3,TSS_2=TSS_2,ReadRanges,WindowSizeLeft,WindowSizeRight,mc.cores=4)
#     print(i)
  #   return(Temp)
#  }
...
library(GenomicRanges)
print(Arguments[3])
if(tolower(Arguments[3]) %in% tolower("HG18")){
  load("/lustre/mib-cri/carrol09/Work/MyPipe/Annotation/Genes_HG18.RData")
  Genes <- Genes36
}

if(tolower(Arguments[3]) %in% tolower("GRCh37")){
...
CoverageFeature(fileForCov,4000,1000,"TSS_AvCov_",Genes,DirectoryOut)