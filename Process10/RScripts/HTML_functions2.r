...
LinkMyGoogle <- function(ss,Charts,Table,chartOptions,chartTypes){
...
googleData <- paste(asJSVars(googleData=JSON_Obj,qualifier="var"),"\n")
sortData <- "var sortData = new google.visualization.DataTable(googleData);\n"
sortData2 <- "var sortData2 = new google.visualization.DataView(sortData);\n"
StartVisualisation <- "function drawVisualization() {\n"
MakeTable <- "var table = new google.visualization.Table(document.getElementById(\"table\"));\ntable.draw(sortData2, {allowHtml:true});\n"
TableIndexInMain <-  which(colnames(ss) %in% Table)-1
SetTable <- paste("sortData2.setColumns(",toJSON(TableIndexInMain),");\n",sep="")
ChartList <- vector("list",length=length(Charts))
for(i in 1:length(Charts)){
    IndexesInMain <-  which(colnames(ss) %in% Charts[[i]])-1
    ChartList[[i]] <- paste("var ",names(Charts)[i]," = new google.visualization.DataView(sortData);\n",names(Charts)[i],".setColumns(",toJSON(IndexesInMain),");\n
...
TopOfSortEvent <- "google.visualization.events.addListener(table, \'sort\',\n
function(event) {\n
...
GoogleVisMain <- function(myGoogleVis,p){
  TempTag <- paste(hmakeTag("Content",data=googleStuff@jsScript))
...
MakeIGVSessionXML <- function(files,fileType,names,igvdirectory,XMLname,genomeName,locusName="All"){
  ...
  if(fileType == "Bam"){
   ...
   if(fileType == "Macs" | fileType == "Tpics"| fileType == "Sicer"){
...
googleStuff <- LinkMyGoogle(ss,Charts,Table,chartOptions,chartTypes)
