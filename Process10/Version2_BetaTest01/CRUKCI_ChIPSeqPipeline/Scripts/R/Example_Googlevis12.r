...
library(scales)
library(gridSVG)
source("/lustre/mib-cri/carrol09/Work/MyPipe/Process10/RScripts/HTML_functions15.r")
...
SSMPMain <- MakeSSMP(SampleSheet,genome,chartOptions)
...
hwrite("",br=T,p)
addVisElement("AbsPeaksInReads",p)
hwrite("Figure 11 - The number of reads overlapping or not overlapping the MACs peak regions",br=T,p) 
hwrite("",br=T,p)
addVisElement("PercentPeaksInReads",p)
hwrite("Figure 12 - The percentage of reads overlapping or not overlapping the MACs peak regions",br=T,p) 
...
hwrite("Figure 13 - The number of base pairs in Log2 at given read depths within the MACs peak regions and outside them",br=T,p) 
...
hwrite("Figure 14 - Boxplot of the GC content of MACs peak regions between samples",br=T,p) 
...
addImage("PosAndNegRatioCoverage.png",file.path(WkgDir,"HTML_Report","Plots","GinisPlot.png"),"","",p)
...
hwrite("Figure 15 - Boxplot of the  ratio of reads from the positive or negative strand which overlap MACs peak regions",br=T,p) 
cat(file=p,paste(Divs1[[3]][2],"\n",sep=""))
hwrite("",br=T,p)
