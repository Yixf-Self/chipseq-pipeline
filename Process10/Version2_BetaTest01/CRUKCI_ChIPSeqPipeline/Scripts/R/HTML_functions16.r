...
for (i in 1:length(Histfiles)){
  ...
  print(paste("Merging hist from ",Histfiles[i],sep=""))
}
colnames(ToMerge) <- c("Depth",gsub("\\.bwa.*","",forPic))
print("Making standard plot")
...
GetGCInPeaks <-function(SampleSheet,Caller="Macs"){
ss <- SampleSheet
...
GetPeakProfilePlot <- function(SampleSheet,Caller="Macs"){
...
#save(p,file=file.path(WkgDir,"Coverage","AverageTSSPlot.RData"))
ggsave(p,file=file.path(WkgDir,"Coverage",paste(Caller,"_AveragePeakTSSPlot.png",sep="")),width=20,height=8)
}
...
MakeOnOffPeakPics <- function(Caller="Macs"){
...
ggsave(file=file.path(WkgDir,"HTML_Report","Plots",paste(Caller,"_","OnAndOffPeakCoverage.png",sep="")))
}
...
CountPeaks <- function(HTMLreport,Name,MacsFile){
if(MacsFile != "" & file.exists(MacsFile)){
 PeaksFile <- read.delim(MacsFile,comment="#",sep="\t",header=F)
...
#  PerSampleReports[i] <- MakePerSampleMacsPeaks(file.path(WkgDir,"HTML_Report","SampleSummary.html"),SampleSheet[i,"GenomicsID"],Macs[i])
      PeakTableLists[i] <- paste("<a href=\"file:",MakePerSampleMacsPeaks(file.path(WkgDir,"HTML_Report","SampleSummary.html"),SampleSheet[i,"GenomicsID"],Macs[i]),"\">",paste(SampleSheet[i,"SampleName"],"_Peaks_WithLinks.csv",sep=""),"</a>",sep="")
  print(i)
}

for(i in 1:nrow(SampleSheet)){
#  PerSampleReports[i] <- MakePerSampleMacsPeaks(file.path(WkgDir,"HTML_Report","SampleSummary.html"),SampleSheet[i,"GenomicsID"],Macs[i])
      PeakCounts[i] <- CountPeaks(file.path(WkgDir,"HTML_Report","SampleSummary.html"),SampleSheet[i,"GenomicsID"],Macs[i])
  print(i)
}
...
#  PeakTableLists <- paste("<a href=",PerSampleReports,">",SampleSheet[,"MacsPeaks"],"</a>",sep="")
  ...
  SSMP4[,3] <- paste("<a href=\"file:",relativePath(SSMP4[,3],file.path(WkgDir,"HTML_Report","HTMLReport.html")),"\">",paste(SampleSheet[i,"SampleName"],"_GenesAndPeaksTable.csv",sep=""),"</a>",sep="")
  SSMP4[,5] <- paste("<a href=\"file:",relativePath(SSMP4[,5],file.path(WkgDir,"HTML_Report","HTMLReport.html")),"\">",paste(SampleSheet[i,"SampleName"],"_GoTable.csv",sep=""),"</a>",sep="")
